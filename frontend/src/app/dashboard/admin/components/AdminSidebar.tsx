...
const AdminSidebar = ({ isCollapsed, onHover }: {
  isCollapsed: boolean,
  ...
  useGSAP(() => {
    if (isCollapsed && activeDesignRef.current) {
      gsap.fromTo(activeDesignRef.current, {
        ...
        delay: 0.5,
        ease: 'power2.out',
        clearProps: "all" // Cleans up transforms that interfere with 'fixed' children
      })
    }
  }, [isCollapsed]);
    ...
    <nav className={`space-y-2  ${isCollapsed ? 'flex flex-col items-center gap-2' : ''}`}>
      {adminMenuItems.map((item) => {
        ...
        return (
          <div
            key={item.href}
            className={`relative group w-full  ${isCollapsed ? 'flex flex-col items-center justify-center gap-4 h-16' : ''}`}
